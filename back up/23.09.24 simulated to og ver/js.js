...
clickedtile(color);
//this gamveoverscreen is not really required, but is added just to match with the original challenge version
gameoverscreen();
}
//closing for the click event handler for the tempgreyingon function
})
//closing for the tempgreyon function
}
...
//items should become clickable only on not gameover, which resets the counters
 $('.btn').on("click", (e)=>{
...
if (tempplayarr.length > levelarr.length) {
  //can optionally turn off click event on gameover, but is disabled to match with orginal challenge template
  // $('.btn').off("click");
  gameoverscreen();
  break;
...
//break is used to avoid excessive empty iterations after tempplayarr.length is reached
else {break;}};
